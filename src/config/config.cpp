...
Config Config::from_file( const std::string& path ) {
    ...
    Config cfg ;
    std::ifstream f(path) ;
    if (!f.is_open()) return cfg ;  // if file not opened fallback to defaults
    ...
    while ( std::getline( f, line ) ) {
        ...
        else if ( key == "snapshot_interval" ) cfg.snapshot_interval = std::stoi(val) ;
        else if ( key == "rdb_path" ) cfg.rdb_path = val ;
        else if ( key == "max_connections" ) cfg.max_connections = std::stoul(val) ;
    }
    return cfg ;
}
